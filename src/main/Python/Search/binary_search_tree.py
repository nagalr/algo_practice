...
class BinarySearchTree:
    ...
    def dfs_inorder(self, node):
        """
        Time O(n) run, traverse over all nodes.
        O(log n) space-complexity - measure on the memory Stack.
        the height of the tree will match the deepest recursive function,
        Hence O(log n) space needed in the Stack for this method,
        as well as for the following two methods.
        """
        if node is None:
            return
